    ...
    <Wrap ref={WrapBg}>
      ...
      <Paper ref={bg}>
        ...
        <InText>
          ...
          역동적이고 심플한 포트폴리오용 사이트를 보여주고 싶었습니다.<br /><br />
          <MovieImg src={portImg} alt="ChartImg" />
          이번 작업을 진행하며 막혔었던 점은<br /><br />
          1. 렌더시 fade-in-out 효과 시 화면 깜빡임 현상으로 인한 불편한 사용자 경험<br /><br />
          2. img들의 느린 렌더링으로 인한 불편한 사용자 경험<br /><br /><br />
          1번째 문제를 해결한 방법은 useEffect가 아닌 useLayoutEffect를 사용한 방법입니다.<br /><br />
          useEffect의 이펙트는 DOM이 화면에 그려진 이후에 호출이됩니다.<br /><br />
          허나 useLayoutEffect의 이펙트는 DOM이 화면에 그려지기 전에 호출됩니다.<br /><br />
          공식문서에선 useEffect의 사용만을 선호하고있지만 useEffect에 dom에 영향을 주는 코드가 있을 경우 useLayoutEffect를 사용해야 특정dom 렌더시 깜빡임 현상이 해결된다는 것을 알았습니다.<br /><br /><br />
          2번째 문제를 해결한 방법은 public 폴더가 아닌 asset폴더에 img를 넣은 방법입니다.<br /><br />
          이 경우는 컴파일 시에 필요한 파일인지 아닌지에 따라 달라졌습니다.<br /><br />
          파비콘과 같은 index.html 내부에서 직접 사용하여 컴파일 단계가 필요 없는 파일들은 public파일에 저장하고 컴포넌트 내부에서 사용해야 하는 이미지 파일 같은 경우는 asset 폴더에 위치시켰습니다.<br /><br />
        </InText>
      </Paper>
    </Wrap>
  );
